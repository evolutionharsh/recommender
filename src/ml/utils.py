...
def train_surprise_model(n_epochs=20 , verbose=True):
    ...
    cv_results = cross_validate(model, loaded_data, measures=['RMSE','MAE'],
                     cv=4, verbose=True)
    ...
    export_model(model, model_name, model_type='surprise' ,
                  model_ext='pkl' , verbose=True)

def export_model(model, model_name = 'model' , model_type='surprise' ,
                  model_ext='pkl' , verbose=True):
