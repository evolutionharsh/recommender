...
def generate_fake_reviews(count=100, users=10 , null_avg=False):
    ...
    random_user_ids = random.sample(range(user_s.id,
                                          user_e.id), users)
...
def task_update_movie_ratings(object_id = None):
    ...
    agg_ratings = rating_qs.values('object_id').annotate(average=Avg('value'),
    ...
    for agg_rate in agg_ratings:
        ...
        score = decimal.Decimal(agg_rate['average'] * rating_count * 1.0)
        #print(score)
