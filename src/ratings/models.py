...
# user_ratings = user_obj.rating_set.all()  # Reverse relationship
...
class Rating(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.
                             CASCADE)
    value = models.IntegerField(null=True, blank=True, choices
...
def rating_post_save(sender, instance, created, *args, **kwargs):
     if created:
         ...
         if instance.active:
              qs = Rating.objects.filter(content_type=instance.content_type,
                                         ...
                                         user = instance.user).exclude(id=_id, active = True)
              if qs.exists():
                  qs = qs.exclude(active_update_timestamp_isnull=False)
     ...
     suggestion_qs = Suggestion.objects.filter(content_type=instance.content_type,
                                         object_id = instance.object_id,
                                         user = instance.user, did_rate=False,)
